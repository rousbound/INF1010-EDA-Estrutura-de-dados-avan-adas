...
int main(int argc, char **argv) {
  ...
  for (i=tammapa;i>0;i--) m4 = insere(m4, i, dados(i));
  ...
  for (i=0;i<6;i++) m3 = insere(m3, m3_dados[i], dados(m3_dados[i]));
	...
	m3 = balancear(m3);
	printf("m3 após função de balancear, é balanceada?: %s\n",e_balanceada(m3)? "Não": "Sim");
	//mostra(m3);
	printf("\n\n--------------\n\n");

	printf("m4 é balanceada?: %s\n",e_balanceada(m4)? "Não": "Sim");
	m4 = balancear(m4);
	
	printf("m4 após função de balancear, é balanceada?: %s\n",e_balanceada(m4)? "Não": "Sim");
	...
	printf("\n\n\n");
	//mostra(m1);
	printf("m1 após função de balancear, é balanceada?: %s\n",e_balanceada(m1)? "Não": "Sim");
	printf("Altura de m1: %d\n",altura(m1));
